...
import { IActorsSerials, IGenreItem, IGenreMovies, IGenreSerials } from '../../utils/types';
import { useNavigate } from 'react-router-dom';
import http from '../../http';

export const ActorsSerials = () => {
    const [allGenres, setAllGenres] = useState<IActorsSerials[]>([]);
    const [error, setError] = useState<string>('');
    const [addingGenre, setAddingGenre] = useState<boolean>(false);
    const [editingId, setEditingId] = useState<number | null>(null);
    const [editingGenreId, setGenreId] = useState<string>("");
    const [addGenreMovies, setGenreMovies] = useState<IActorsSerials>({
        id: 0,
        actorId: "",
        serialId: "",
        actors_name: "",
    ...
    const fetchDirectors = () => {
        http.get<IActorsSerials[]>('/actorSerials/index')
            .then(resp => {
    ...
    const handleNewDir = async () => {
        await http
            .post<IActorsSerials>("/actorSerials/store", addGenreMovies, {
                headers: {
    ...
    const handleDelete = (idD: number) => {
        http.post(`/actorSerials/delete`, { id: idD })
            .then(() => {
                fetchDirectors();
            })
            .catch((error) => {
                    ...
                    <table className="table table-hover tm-table-small tm-product-table">
                        <thead>
                            <tr>
                                ...
                                <th scope="col">Serial Title</th>
                                <th scope="col">&nbsp;</th>
                            </tr>
                        </thead>
                        <tbody>
                            {allGenres.map((item: IActorsSerials) => (
                                <tr key={item.id}>
                            ...
                            {addingGenre && (
                                <tr>
                                    ...
                                    <td>
                                        <input type="text" value={addGenreMovies.serialId} name='serialId' onChange={handleChange} className="form-control" placeholder="Enter serial id" />
                                    </td>
                                    <td>
